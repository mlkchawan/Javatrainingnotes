...
def create_and_load_table(**kwargs):
    ...
    try:
        ...
        df.to_sql(name='zuora_daily_adds',con=engine, if_exists='replace', index=False, schema='reportsdb',chunksize=1000, method='multi')
        #Add Primary Key Constraint:
        cursor = pg_hook.cursor()  
        grant_query = ''' GRANT SELECT ON reportsdb."zuora_daily_adds" TO sailakshmir,laxmikanthm,arunkumark,rgarikapati,mayankb,areddy,kmudragada; '''
        ...
        pk_check_query = f'''
            ...
            AND table_name = 'zuora_daily_adds';
        '''
        cursor.execute(pk_check_query)                    
        primary_key_count = cursor.fetchone()[0]
        if primary_key_count == 0:
            alter_query=f'''ALTER TABLE reportsdb.zuora_daily_adds ADD PRIMARY KEY ("RatePlanId");'''
        ...
        for _, row in df.iterrows():
            ...
            if len(rows) == batch_size:
                upsert_query = f'''
                    INSERT INTO reportsdb.zuora_daily_adds({", ".join(['"{}"'.format(col) for col in df.columns])})
...
#Create a DAG
dag = DAG(
    dag_id='zuora_to_pg_Daily_Adds_production',
    default_args=default_args,
    description='Extract data from zuora and load into postgres',
    schedule_interval='@daily',
    tags = ['zuora','production','DailyAdds']
)
