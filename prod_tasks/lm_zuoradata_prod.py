#working Python code for pulling 1years data from zuora and loading to PG "newzuoradata"
#date: 05/02/2024
    ...
    try:
        ...
        query_payload = {
            "queries": [
                        {
                        "name": "zuoradata",
                        "query": f'''select RatePlan.Id, RatePlan.Name , RatePlan.AmendmentType, RatePlan.CreatedDate, RatePlan.UpdatedDate,
                            ...
                            from RatePlanCharge where RatePlan.createdDate >= '2023-01-01T00:00:00Z' and Subscription.Version = 1 and Subscription.Currency='USD' and ((not Subscription.displayName__c like 'yqa%' and not Subscription.displayName__c like 'ysbsqa%') and (not Account.userId__c like 'yqa%'))
                            ...
                            Subscription.CreatedDate, Subscription.UpdatedDate ''',
                        "type": "zoqlexport"
                        }
                        ]
            }        
            ...
            if status == 'completed':
                ...
                if recordCount != 0:
                    ...
                    if result_response.status_code == 200:
                            ...
                            df['RateplanDate'] = pd.to_datetime(df['Rate Plan: Created Date'],utc=True)
                            df['ReportDate']=df['RateplanDate'].dt.date
                            df.drop(['RateplanDate'], axis=1, inplace=True)
                            df.fillna(' ')
...
#Create a DAG
dag = DAG(
    dag_id='zuora_to_postgreszuoradata_prod',
