    ...
    try:
        ...
        query_payload = {
            "queries": [
                        {
                        "name": "Rateplancharge",
                        "query": f''' select Invoice.Amount as InvoiceAmount,Invoice.DueDate as InvoiceDueDate,Invoice.Id as InvoiceId,
                                    Invoice.InvoiceNumber as InvoiceNumber,
                                    Invoice.PostedDate as PostedDate,Invoice.RefundAmount as InvoiceRefundAmount,Invoice.Status as InvoiceStatus,
                                    Invoice.TargetDate as InvoiceTargetDate,Invoice.TaxAmount as TaxAmount,Invoice.PaymentTerm as PaymentTerm,
                                    Invoice.NextActionDate__c as NextActionDate,Invoice.treatmentLevel__c as treatmentLevel,Invoice.InCollection__c as InvoiceInCollection,
                                    Invoice.RetryStatus__c as InvoiceRetryStatus,
                                    ...
                                    Payment.Updateddate as Payment_UpdatedDate,
                                    ...
                                    from InvoicePayment where Invoice.PostedDate >='{last_processed_value}' and Invoice.Currency='USD' Order by Invoice.PostedDate ASC
                            ''',
                        "type": "zoqlexport"
                        }
                        ]
            ...
            if status == 'completed':
                ...
                if recordCount != 0:
                    ...
                    if result_response.status_code == 200:
                        ...
                        last_processed_value = df['PostedDate'].iloc[-1]
                        cursor = pg_hook.cursor()
                        try:
...
def create_and_load_table(**kwargs):
    ...
    try:
        ...
        if primary_key_count == 0:
            alter_query=f'''ALTER TABLE reportsdb.zuora_payment ADD PRIMARY KEY ("PaymentId","InvoiceId");'''
        ...
        for _, row in df.iterrows():
            ...
            if len(rows) == batch_size:
                upsert_query = f'''
                    ...
                    ON CONFLICT ("PaymentId","InvoiceId") DO UPDATE SET
