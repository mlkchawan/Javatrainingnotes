#Python code for pulling renenueforecast data from zuora and loading to PG "zuora_revenueforecast2024"
#date: 28/03/2024
...
def get_last_processed_value(**kwargs):
    ...
    try:
        cursor.execute(f"SELECT value FROM reportsdb.tbl_zuora_metadata WHERE object = 'contact'")
        result = cursor.fetchone()
        return result[0] if result else None
    finally:
        cursor.close()

def extract_contact_from_zuora(**kwargs):
    ...
    try:
        ...
        query_payload = {
            "queries": [
                        {
                        "name": "contact",
                        "query": f'''select Id as Id,Accountid as Accountid,CreatedDate as CreatedDate,UpdatedDate as UpdatedDate,
                                Country as Country,State as State,City as City,County as County,PostalCode as PostalCode,
                                WorkEmail as WorkEmail from contact where CreatedDate > '{last_processed_value}' order by CreatedDate ASC Limit 500000 ''',
                        "type": "zoqlexport"
                        }
                        ]
            }        
                ...
                if recordCount != 0:
                    ...
                    # Fetch the date from the last row
                        ...
                        try:
                            ...
                            cursor.close()                            
                    #Load it to xcom
                        kwargs['ti'].xcom_push(key='zuora_contact', value=df)
...
def check_records(**kwargs):
    df = kwargs['ti'].xcom_pull(task_ids="extract_contact_from_zuora",key='zuora_contact')    
...
def create_and_load_table(**kwargs):
    ...
    try:
        ...
        df.to_sql(name='zuora_contact',con=engine, if_exists='append', index=False, schema='reportsdb',chunksize=1000, method='multi')
        #Add Primary Key Constraint:
        cursor = pg_hook.cursor()  
        grant_query = ''' GRANT SELECT ON reportsdb."zuora_contact" TO sailakshmir,laxmikanthm,arunkumark,rgarikapati,mayankb,areddy,kmudragada; '''
        ...
        pk_check_query = f'''
            ...
            AND table_name = 'zuora_contact';
        '''
        cursor.execute(pk_check_query)                    
        primary_key_count = cursor.fetchone()[0]
        if primary_key_count == 0:
            alter_query=f'''ALTER TABLE reportsdb.zuora_contact ADD PRIMARY KEY ("Id");'''
...
extract_data = PythonOperator(
    task_id='extract_contact_from_zuora',
    python_callable=extract_contact_from_zuora,
    provide_context=True,
    dag=dag,
)
