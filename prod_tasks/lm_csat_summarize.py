...
def load_data_to_postgres(**kwargs):
    ...
    try:
        ...
        df.to_sql(name='CSAT_Summarytable_Salesforce', con=engine, if_exists='replace', index=False, schema='salesforce_airflow',chunksize=1000, method='multi')  
        cursor = pg_hook.cursor()
 
        #Add Primary Key Constraint:
        grant_query = ''' GRANT SELECT ON salesforce_airflow."CSAT_Summarytable_Salesforce" TO sailakshmir,laxmikanthm,arunkumark,rgarikapati,areddy; '''
        ...
        for _, row in df.iterrows():
            ...
            if len(rows) == batch_size:
                upsert_query = f'''
                    INSERT INTO salesforce_airflow.CSAT_Summarytable_Salesforce({", ".join(['"{}"'.format(col) for col in df.columns])})
...
dag = DAG(
    'postgres_data_csat_summary_table',
