# Load Tables in Salesforce to Postgres.
# Created By:Mayank Bhardwaj
# Date : 02/01/2024
# Version: 5
# Source-Environment : salesforce production 
# Target-Environment : prod datawarehouse DB
# Description : This dag fetches all the data for "SurveyGizmo_Survey_Response__c" object of Salesforce.  
#               It runs an SQL query referencing value of object from salesforce_airflow.sf_metadata in postgres for incremental load.
#               Thereafter data is inserted into postgres. Upon insertion metadata table is updated for the latest value. 
...
dag = DAG(
    'sf_to_pg_SurveyGizmo_Survey_Response__c_production',
    default_args=default_args,
    description='DAG to incrementally load Salesforce SurveyGizmo_Survey_Response__c data to PostgreSQL',
    tags = ['production','salesforce','16','SurveyGizmo_Survey_Response__c'],
    schedule_interval='0 16 * * *',
    catchup=False, 
)

# Define the sf_etl_SurveyGizmo_Survey_Response__c task
salesforce_to_postgres_task = sf_etl_SurveyGizmo_Survey_Response__c(
    task_id='sf_to_pg_SurveyGizmo_Survey_Response__c_prod',
    salesforce_conn_id='salesforce-prod',
    postgres_conn_id='warehouse-prod-mb',
    salesforce_object='SurveyGizmo_Survey_Response__c',
    execution_timeout=timedelta(hours=4),    
    dag=dag,
)

# Set up task dependencies
salesforce_to_postgres_task  # No dependencies