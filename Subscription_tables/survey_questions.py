...
def extract_and_load_to_dataframe(**kwargs):
    ...
    mysql_query = 'SELECT * FROM survey_questions '
...
def create_and_load_table(**kwargs):
    ...
    try:
        df.to_sql(name='sub_survey_questions',con=engine, if_exists='replace', index=True, schema='reportsdb',chunksize=None, method='multi')
...
#Create a DAG
dag = DAG(
    dag_id='mysql_to_postgres_surveyquestions',
