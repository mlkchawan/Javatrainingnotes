...
def create_and_load_table(**kwargs):
    ...
    if df is None or df.empty:
        return
    try:
        df.to_sql(name='sub_store_itemcount',con=engine, if_exists='append', index=False, schema='reportsdb',chunksize=1000, method='multi')
...
#Create a DAG
dag = DAG(
    dag_id='subdb_to_pg_store_itemcount_prod',
    default_args=default_args,
    description='Extract data from MySQL and load into postgres',
    schedule_interval='@daily',
    tags = ['mysql','production','storeitemcount'],
) 
