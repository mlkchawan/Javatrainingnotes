...
def get_last_processed_value():
    ...
    try:
        cursor.execute(f"SELECT value FROM reportsdb.sub_metadata WHERE object = 'billproviderorder'")
...
def extract_and_load_to_dataframe(**kwargs):
    ...
    # mysql_query =f"SELECT * FROM bill_provider_order WHERE date(created_at) >= '2022-01-01' Order by created_at ASC"
    mysql_query =f"select * from bill_provider_order where date(created_at) > '{last_processed_value}' order by created_at ASC ;"
    ...
    try:
        if not df.empty:
            ...
            cursor = pg_hook.cursor()
            try:
                cursor.execute(f"INSERT INTO reportsdb.sub_metadata (object, value) VALUES ('billproviderorder', %s) ON CONFLICT (object) DO UPDATE SET value = %s", (last_processed_value, last_processed_value))
                pg_hook.commit()
            finally:
                cursor.close()
        else:
    ...
    kwargs['ti'].xcom_push(key='sub_bill_provider_order_prod', value=df)
    
    #close the connection
    connection.close()
       
def check_records(**kwargs):
    df = kwargs['ti'].xcom_pull(task_ids="extract_and_load_to_dataframe",key='sub_bill_provider_order_prod')    
...
def create_and_load_table(**kwargs):
    ...
    if df is None or df.empty:
        return
    try:
        df.to_sql(name='sub_bill_provider_order',con=engine, if_exists='append', index=False, schema='reportsdb',chunksize=1000, method='multi')
...
#Create a DAG
dag = DAG(
    dag_id='subdb_to_pg_billproviderorder_prod',
    default_args=default_args,
    description='Extract data from MySQL and load into postgres',
    schedule_interval='@daily',
    tags = ['mysql','production','billprovider'],
) 
