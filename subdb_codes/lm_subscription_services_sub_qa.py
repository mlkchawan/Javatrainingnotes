...
def get_last_processed_value():
    ...
    try:
        cursor.execute(f"SELECT value FROM reportsdb.sub_metadata WHERE object = 'sub_sub_services'")
...
def extract_and_load_to_dataframe(**kwargs):
    ...
    # mysql_query =f"SELECT * FROM subscription_services WHERE created_at >= CURDATE() - INTERVAL 2 YEAR"
    ...
    try:
        if not df.empty:
            ...
            cursor = pg_hook.cursor()
            try:
                cursor.execute(f"INSERT INTO reportsdb.sub_metadata (object, value) VALUES ('sub_sub_services', %s) ON CONFLICT (object) DO UPDATE SET value = %s", (last_processed_value, last_processed_value))
                pg_hook.commit()
            finally:
                cursor.close()
        else:
...
#Create a DAG
dag = DAG(
    dag_id='subdb_to_pg_subscription_services_sandbox',
