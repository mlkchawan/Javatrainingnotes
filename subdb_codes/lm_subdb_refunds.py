#Python code for Extracting data from mysql and loading into Postgres "sub_refunds"
#date: 22/03/2024
...
def get_last_processed_value():
    ...
    try:
        cursor.execute(f"SELECT value FROM reportsdb.sub_metadata WHERE object = 'sub_refunds'")
...
def extract_and_load_to_dataframe(**kwargs):
    ...
    mysql_query =f"SELECT * FROM refunds WHERE date(created_at) >= '2022-01-01' order by created_at ASC "
    # mysql_query =f"select * from refunds where date(created_at) >= '{last_processed_value}' order by created_at ASC;"
    df = mysql_hook.get_pandas_df(mysql_query)
    
    flattened_data = []
    for _, row in df.iterrows():
        json_data = json.loads(row['invoice_items'])
        for item in json_data:
            flattened_item = {
                ...
                'prorated_invoice_number': row['prorated_invoice_number'],
                'amount': row['amount'],
                ...
                'status': row['status'],
                'service_start_date': item.get('serviceStartDate'),
                'service_end_date': item.get('serviceEndDate'),
                'charge_amount': item.get('chargeAmount'),
                'available_amount': item.get('availableAmount'),
                'charge_description': item.get('chargeDescription'),
                'charge_name': item.get('chargeName'),
                ...
                'invoice_item_id': item.get('invoiceItemId'),
                'subscription_number': item.get('subscriptionNumber'),
    ...
    df['CreatedDate'] = pd.to_datetime(df['created_at'], errors='coerce').dt.date
    try:
        if not df.empty:
            last_processed_value = df['created_at'].iloc[-1]
            cursor = pg_hook.cursor()
            try:
                cursor.execute(f"INSERT INTO reportsdb.sub_metadata (object, value) VALUES ('sub_refunds', %s) ON CONFLICT (object) DO UPDATE SET value = %s", (last_processed_value, last_processed_value))
                pg_hook.commit()
            finally:
                cursor.close()
        else:
    ...
    kwargs['ti'].xcom_push(key='sub_refunds', value=df)
    
    #close the connection
    connection.close()
       
def check_records(**kwargs):
    df = kwargs['ti'].xcom_pull(task_ids="extract_and_load_to_dataframe",key='sub_refunds')    
...
def create_and_load_table(**kwargs):
    ...
    if df is None or df.empty:
        return
    try:
        df.to_sql(name='sub_refunds',con=engine, if_exists='replace', index=False, schema='reportsdb',chunksize=1000, method='multi')
        #Add Primary Key Constraint:
        cursor = pg_hook.cursor() 
        grant_query = ''' GRANT SELECT ON reportsdb."sub_refunds" TO sailakshmir,laxmikanthm,arunkumark,rgarikapati,mayankb,areddy,kmudragada; '''
        ...
        #     AND table_name = 'sub_refunds';
        # '''
        # cursor.execute(pk_check_query)                    
        # primary_key_count = cursor.fetchone()[0]
        # if primary_key_count == 0:
        #     alter_query=f'''ALTER TABLE reportsdb.sub_refunds ADD PRIMARY KEY ("id");'''
        ...
        #         upsert_query = f'''
        #             INSERT INTO reportsdb.sub_refunds({", ".join(['"{}"'.format(col) for col in df.columns])})
...
#Create a DAG
dag = DAG(
    dag_id='subdb_to_pg_sub_refunds_production',
    default_args=default_args,
    description='Extract data from MySQL and load into postgres',
    schedule_interval='@daily',
    tags = ['mysql','production','refunds'],
) 
