#Python code for Extracting data from mysql and loading into Postgres "cancel_save_history"
#date: 27/03/2024
...
def get_last_processed_value():
    ...
    try:
        cursor.execute(f"SELECT value FROM reportsdb.sub_metadata WHERE object = 'cancelsave_history'")
...
def extract_and_load_to_dataframe(**kwargs):
    ...
    # mysql_query =f"SELECT * FROM cancel_save_history where date(created_at) > '2022-01-01' Order by created_at ASC"
    mysql_query =f"select * from cancel_save_history where date(created_at) > '{last_processed_value}' order by created_at ASC;"
    ...
    try:
        if not df.empty:
            ...
            cursor = pg_hook.cursor()
            try:
                cursor.execute(f"INSERT INTO reportsdb.sub_metadata (object, value) VALUES ('cancelsave_history', %s) ON CONFLICT (object) DO UPDATE SET value = %s", (last_processed_value, last_processed_value))
                pg_hook.commit()
            finally:
                cursor.close()
        else:
    ...
    kwargs['ti'].xcom_push(key='sub_cancel_save_history', value=df)
    
    #close the connection
    connection.close()
       
def check_records(**kwargs):
    df = kwargs['ti'].xcom_pull(task_ids="extract_and_load_to_dataframe",key='sub_cancel_save_history')    
...
def create_and_load_table(**kwargs):
    ...
    if df is None or df.empty:
        return
    try:
        df.to_sql(name='sub_cancel_save_history',con=engine, if_exists='append', index=False, schema='reportsdb',chunksize=1000, method='multi')
        #Add Primary Key Constraint:
        cursor = pg_hook.cursor() 
        grant_query = ''' GRANT SELECT ON reportsdb."sub_cancel_save_history" TO sailakshmir,laxmikanthm,arunkumark,rgarikapati,mayankb,areddy,kmudragada; '''
        ...
        pk_check_query = f'''
            ...
            AND table_name = 'sub_cancel_save_history';
        '''
        cursor.execute(pk_check_query)                    
        primary_key_count = cursor.fetchone()[0]
        if primary_key_count == 0:
            alter_query=f'''ALTER TABLE reportsdb.sub_cancel_save_history ADD PRIMARY KEY ("id");'''
        ...
        for _, row in df.iterrows():
            ...
            if len(rows) == batch_size:
                upsert_query = f'''
                    INSERT INTO reportsdb.sub_cancel_save_history({", ".join(['"{}"'.format(col) for col in df.columns])})
...
#Create a DAG
dag = DAG(
    dag_id='subdb_to_pg_cancel_save_history_prod',
    default_args=default_args,
    description='Extract data from MySQL and load into postgres',
    schedule_interval='@daily',
    tags = ['mysql','production','history'],
) 
