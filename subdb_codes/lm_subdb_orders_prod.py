...
def get_last_processed_value():
    ...
    try:
        cursor.execute(f"SELECT value FROM reportsdb.sub_metadata WHERE object = 'sub_orders'")
...
def extract_and_load_to_dataframe(**kwargs):
    ...
    # mysql_query =f"SELECT id,status,user_id,cart_id,created_at,effective_at,modified_at FROM orders WHERE date(created_at) >= '2022-01--01' Order by created_at ASC"
    mysql_query =f"select id,status,user_id,cart_id,created_at,effective_at,modified_at from orders where created_at >'{last_processed_value}' order by created_at ASC"
    ...
    try:
        if not df.empty:
            ...
            cursor = pg_hook.cursor()
            try:
                cursor.execute(f"INSERT INTO reportsdb.sub_metadata (object, value) VALUES ('sub_orders', %s) ON CONFLICT (object) DO UPDATE SET value = %s", (last_processed_value, last_processed_value))
                pg_hook.commit()
            finally:
                cursor.close()
        else:
...
def create_and_load_table(**kwargs):
    ...
    if df is None or df.empty:
        return
    try:
        df.to_sql(name='sub_orders',con=engine, if_exists='append', index=False, schema='reportsdb',chunksize=1000, method='multi')
