...
def extract_and_load_to_dataframe(**kwargs):
    ...
    mysql_query =f"SELECT * FROM extra_services WHERE date(created_at) >= '2023-06-01' Order by created_at ASC"
...
def create_and_load_table(**kwargs):
    ...
    if df is None or df.empty:
        return
    try:
        df.to_sql(name='sub_extra_services',con=engine, if_exists='replace', index=False, schema='reportsdb',chunksize=1000, method='multi')
