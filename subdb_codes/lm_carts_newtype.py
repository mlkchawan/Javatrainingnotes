...
def extract_and_load_to_dataframe(**kwargs):
    ...
    for _, row in result.iterrows():
        ...
        record = {
            ...
            # 'bussiness_id' : business_id if business_id is not None else None
            'business_id' : json_data.get('meta', {}).get('whois', {}).get('businessId')

        }
        # meta_data = json_data.get('meta', {})
        # if 'whois' in meta_data:
        #     whois_data = meta_data['whois']
        #     record['business_id'] = whois_data.get('businessId', '')
        # else:
        #     record['business_id'] = ''
                            ...
                            if onetime_charges:
                                for charge in onetime_charges:
                                    ...
                                    data.append({
                                        ...
                                        'meta_businessId': meta_businessId,
                                        'action': meta_action,
...
def create_and_load_table(**kwargs):
    ...
    try:
        ...
        cursor = pg_hook.cursor()
        grant_query = ''' GRANT SELECT ON reportsdb."sub_carts_new" TO sailakshmir,laxmikanthm,arunkumark,rgarikapati; '''
...
#Create a DAG
dag = DAG(
    dag_id='subdb_to_pg_carts_newtype',
