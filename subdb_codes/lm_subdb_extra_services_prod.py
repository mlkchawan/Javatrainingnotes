#Python code for Extracting data from mysql and loading into Postgres "sub_extra_services"
#date: 01/04/2024
...
def get_last_processed_value():
    ...
    try:
        cursor.execute(f"SELECT value FROM reportsdb.sub_metadata WHERE object = 'sub_extra_services'")
...
def extract_and_load_to_dataframe(**kwargs):
    ...
    # mysql_query =f"SELECT * FROM extra_services WHERE date(created_at) >= '2022-01-01' Order by created_at ASC"
    mysql_query =f"select * from extra_services where date(created_at) >'{last_processed_value}' order by created_at ASC;"
    ...
    try:
        if not df.empty:
            last_processed_value = result['created_at'].iloc[-1]
            cursor = pg_hook.cursor()
            try:
                cursor.execute(f"INSERT INTO reportsdb.sub_metadata (object, value) VALUES ('sub_extra_services', %s) ON CONFLICT (object) DO UPDATE SET value = %s", (last_processed_value, last_processed_value))
                pg_hook.commit()
            finally:
                cursor.close()
        else:
    ...
    kwargs['ti'].xcom_push(key='sub_extra_services', value=df)
...
def create_and_load_table(**kwargs):
    ...
    if df is None or df.empty:
        return
    try:
        df.to_sql(name='sub_extra_services',con=engine, if_exists='append', index=False, schema='reportsdb',chunksize=1000, method='multi')
        #Add Primary Key Constraint:
        cursor = pg_hook.cursor() 
        grant_query = ''' GRANT SELECT ON reportsdb."sub_extra_services" TO sailakshmir,laxmikanthm,arunkumark,rgarikapati,mayankb,areddy,kmudragada; '''
        ...
        pk_check_query = f'''
            ...
            AND table_name = 'sub_extra_services';
        '''
        cursor.execute(pk_check_query)                    
        primary_key_count = cursor.fetchone()[0]
        if primary_key_count == 0:
            alter_query=f'''ALTER TABLE reportsdb.sub_extra_services ADD PRIMARY KEY ("id");'''
        ...
        for _, row in df.iterrows():
            ...
            if len(rows) == batch_size:
                upsert_query = f'''
                    INSERT INTO reportsdb.sub_extra_services({", ".join(['"{}"'.format(col) for col in df.columns])})
...
#Create a DAG
dag = DAG(
    dag_id='subdb_to_pg_extra_services_prod',
    default_args=default_args,
    description='Extract data from MySQL and load into postgres',
    schedule_interval='@daily',
    tags = ['mysql','Production','extraservices'],
) 
