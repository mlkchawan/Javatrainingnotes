#Python code for Extracting data from mysql and loading into Postgres "subscriptions_services"
#date: 30/01/2024
...
def extract_and_load_to_dataframe(**kwargs):
    ...
    # mysql_query =f"SELECT * FROM subscription_services WHERE date(created_at) >= '2022-01-01'"

    mysql_query =f"select * from subscription_services where date(created_at) >= '{last_processed_value}'; "
    ...
    try:
        if not df.empty:
            ...
            cursor = pg_hook.cursor()
            try:
                cursor.execute(f"INSERT INTO reportsdb.sub_metadata (object, value) VALUES ('sub_subscr_services', %s) ON CONFLICT (object) DO UPDATE SET value = %s", (last_processed_value, last_processed_value))
                pg_hook.commit()
            finally:
                cursor.close()
        else:
    ...
    kwargs['ti'].xcom_push(key='subscription_services', value=df)
    
    #close the connection
    connection.close()
       
def check_records(**kwargs):
    df = kwargs['ti'].xcom_pull(task_ids="extract_and_load_to_dataframe",key='subscription_services')    
...
def create_and_load_table(**kwargs):
    ...
    if df is None or df.empty:
        return
    try:
        df.to_sql(name='sub_subscription_services',con=engine, if_exists='append', index=False, schema='reportsdb',chunksize=1000, method='multi')
        #Add Primary Key Constraint:
        cursor = pg_hook.cursor()
        grant_query = ''' GRANT SELECT ON reportsdb."sub_subscription_services" TO sailakshmir,laxmikanthm,arunkumark,rgarikapati; '''
        ...
        pk_check_query = f'''
            ...
            AND table_name = 'sub_subscription_services';
        '''
        cursor.execute(pk_check_query)                    
        primary_key_count = cursor.fetchone()[0]
        if primary_key_count == 0:
            alter_query=f'''ALTER TABLE reportsdb.sub_subscription_services ADD PRIMARY KEY ("id");'''
        ...
        for _, row in df.iterrows():
            ...
            if len(rows) == batch_size:
                upsert_query = f'''
                    INSERT INTO reportsdb.sub_subscription_services({", ".join(['"{}"'.format(col) for col in df.columns])})
...
dag = DAG(
    ...
    tags = ['mysql','production','services'],
) 
