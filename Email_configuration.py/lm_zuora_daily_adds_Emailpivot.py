            ...
            if status == 'completed':
                ...
                if recordCount != 0:
                    ...
                    if result_response.status_code == 200:
                        ...
                        pivot_product = df_grouped.pivot_table(index=['ReportDate', 'ProductName', 'ProductFamily'], aggfunc={'ProductName': 'count'})
                        pivot_product = pivot_product.rename(columns={'ProductName': 'ProductCount'}).reset_index()
                        ...
                        csv_file_path = '/tmp/zuora_daily_adds_2024.xlsx'
                        ...
                        with pd.ExcelWriter(csv_file_path,engine='openpyxl') as writer:
                            ...
                            df_grouped.to_excel(writer, sheet_name='Dailyadds_RawData',index=False)
                        
                        
                        # df_grouped.to_excel(csv_file_path, index=False)
...
def send_email(**kwargs):
    ...
    email_task = EmailOperator(
        ...
        html_content='The data extraction from zuora for daily adds for this year[2024] has pulled successfully and file attached.',
        files=[csv_file_path],
        dag=dag,
    )
